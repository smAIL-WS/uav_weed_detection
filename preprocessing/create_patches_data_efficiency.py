...
PROCESS_FINAL_CONFIG = True    # ← set to False to skip final_config_data
# ──────────────────────────────────────────────────────────────────────────────


def get_partfield(img_path):
    """Extract partfield letter (A, B, C, D) from image filename."""
    return os.path.basename(img_path).split('_')[2][0]
...
all_img_paths = sorted(glob(os.path.join(TRAIN_IMAGE_ROOT, "**/*.png"), recursive=True))
all_xml_paths = sorted(glob(os.path.join(TRAIN_ANNO_ROOT,  "**/*.xml"), recursive=True))
...
for fold_name, partfields in CV_FOLDS.items():
    ...
    train_img = [p for p in all_img_paths if get_partfield(p) in partfields["train"]]
    train_xml = [p for p in all_xml_paths if get_partfield(p) in partfields["train"]]
    val_img   = [p for p in all_img_paths if get_partfield(p) in partfields["val"]]
    val_xml   = [p for p in all_xml_paths if get_partfield(p) in partfields["val"]]
    ...
    for split in ["val", "test"]:
        process_images(
            val_img, val_xml,
            dst_folder=os.path.join(fold_root, f"{split}_images/"),
            crop_size=CROP_SIZE,
            output_json=os.path.join(fold_root, f"{split}.json")
        )

    # Generate annotation txt files for this fold
...
# ── Final Config Data (optional) ──────────────────────────────────────────────
if PROCESS_FINAL_CONFIG:
    print("\nProcessing final_config_data — all partfields in train")
    fold_root = os.path.join(OUTPUT_ROOT, "final_config_data")

    process_images(
        all_img_paths, all_xml_paths,
        dst_folder=os.path.join(fold_root, "train_images/"),
        crop_size=CROP_SIZE,
        output_json=os.path.join(fold_root, "train.json")
    )
    ...
    # Generate annotation txt files for final_config_data
    for split in ["train", "val", "test"]:
        save_annotation_txt(
            image_folder=os.path.join(fold_root, f"{split}_images/"),
            output_txt=os.path.join(fold_root, f"{split}.txt")
